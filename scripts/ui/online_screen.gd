...
## someone -> they accept -> both end up in the same room.
##
...
func _refresh_list() -> void:
	...
	if others == 0:
		var empty := Label.new()
		empty.text = "No other players online."
...
func _check_invite_answered() -> void:
	...
	if res["ok"] and res["data"] != null:
		_enter_room(_sent_room, _sent_to_name)
...
func _on_accept_pressed() -> void:
	if _busy or _incoming_room == "":
		return
	_busy = true
	var joined: Dictionary = await Net.db_patch("rooms/%s" % _incoming_room, {"guest": Net.uid})
	if joined["ok"]:
...
func _enter_room(code: String, opponent: String) -> void:
	_room_code = code
	_room_opponent = opponent
	_set_stage(Stage.ROOM)
	_room_label.text = "Room %s\nYou vs %s\n\nCountry selection and the match itself are next." % [
		code, opponent]
	_status.text = "In a room."
	# Marks us busy so nobody invites a player who is already occupied.
